...
const buildThreatPayload = (safeData: any, caseNumber: string) => {
  ...
  return {
    ...
    safetyThresholdLookupId: typeof safeData.safetyThreshold === 'string' 
      ? getSafetyThresholdLookupId(safeData.safetyThreshold) 
      : (safeData.safetyThreshold || 2),
...
export const checkThreatAPIHealth = async (): Promise<boolean> => {
  ...
  //   const response = await fetch(`${API_BASE_URL}/PushAddThreatAssessmentWeb`, {
  //     method: 'HEAD',
  //     cache: 'no-cache',
  //   });
...
export const syncThreatAssessment = async (caseNumber: string, assessmentData: any): Promise<ThreatSyncResponse> => {
  ...
  try {
    ...
    const url = `${API_BASE_URL}/PushAddThreatAssessmentWeb?caseNumber=${encodedCaseNumber}`;
